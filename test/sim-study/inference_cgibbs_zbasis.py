...
for pool_size in pool_sizes:
	...
	for ds_idx in range(1, n_datasets+1):
		...
		for n, y in zip(ns, ys):			
			...
			lm.compute_basis(markov=False)
			lm_list.append(lm)
		pre_time = time() - t

		with open(fn_prefix+'.pkl', 'wb') as fp:
			pkl.dump(lm_list, fp)

		print(f'MCMC for set {ds_idx}')
		t = time()
		idata = latent_mult_mcmc_cgibbs(lm_list, H, n_sample, n_burnin, np.ones(H),
		...
		idata.to_netcdf(fn_prefix+'_cgibbs_zbasis.netcdf')
	print()
